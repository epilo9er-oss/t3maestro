...
This guide is for people who want to use more than one Claude setup in T3 Synapse.
...
In T3 Synapse Settings, your Claude provider can stay like this:
...
An empty `Claude HOME path` means T3 Synapse uses your normal home directory.
...
Then add another Claude provider in T3 Synapse:
...
T3 Synapse only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so T3 Synapse keeps separate Claude homes isolated instead of
...
Add or edit a Claude provider in T3 Synapse Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. T3 Synapse stores the value as a server secret and does not
...
T3 Synapse does not need a special Claude Code Router provider. Treat the router as a Claude
environment.
...
2. Add or configure a Claude provider in T3 Synapse.
